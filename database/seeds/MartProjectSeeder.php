...
class MartProjectSeeder extends Seeder
{
    ...
    private function getTimeBasedActivity($hour, $isWeekend, $preferences = [])
    {
        ...
        // Time-based logic
        if ($hour >= 7 && $hour <= 9) return 'Eating'; // Breakfast/morning
        if ($hour >= 9 && $hour <= 17 && !$isWeekend) return 'Working';
        if ($hour >= 17 && $hour <= 19) return 'Commuting';
        if ($hour >= 19 && $hour <= 21) return 'Eating'; // Dinner
        if ($hour >= 22 || $hour <= 6) return 'Relaxing'; // Evening/night
