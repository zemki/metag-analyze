...
export default ({
  ...
  methods: {
    download() {
      ...
      a.href = this.file;
      a.download = `${this.name.substring(this.name.lastIndexOf('/') + 1, this.name.length)}.mp3`; // File name Here
...
.player-controls {
	...
	> div {
		border-right: 1px solid darken(#fff, 12%);
...
.player-progress {
	...
	.player-seeker {
		background-color: darken(#fff, 75%);
		bottom: 0;
		left: 0;
		position: absolute;
		top: 0;
