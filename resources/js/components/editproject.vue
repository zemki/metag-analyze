<template>
    ...
    <div class="content">
      ...
      <div class="level" v-if="projectData.edit">
        ...
        <div class="level-left">
            ...
            <div class="field has-addons" v-if="projectData.edit"><input name="name" v-model="projectData.name" class="input text is-large">
                <div class="control">
                </div>
            </div>
        </div>
        ...
        <div class="level">
            <div class="column is-3">
                ...
                <div class="control" v-for="(m,index) in projectData.media">
                    <input :disabled="!editable" type="text" name="media[]" class="input inputcreatecase" v-model="projectData.media[index]" @keyup="handleMediaInputs(index,m)" autocomplete="off" @keydown.enter.prevent @keydown.tab.prevent>
                </div>
    ...
    <input type="hidden" :value="JSON.stringify(projectData.inputs)" name="inputs">
        <b-field label="Number of additional inputs" :disabled="!editable">
            <b-numberinput :disabled="!editable" name="inputs" id="ninputs" controls-position="compact" type="is-light" min="0" max="3" :editable="false" steps="1" v-model.number="projectData.ninputs"></b-numberinput>
        </b-field>
                ...
                <span v-if="(projectData.inputs[index].type == 'multiple choice' || projectData.inputs[index].type == 'one choice')">
                    <div class="field">
                        ...
                        <div class="control" v-for="(m,answerindex) in projectData.inputs[index].answers">
                            <input type="text" class="input inputcreatecase" v-model="projectData.inputs[index].answers[answerindex]" @keyup="handleAdditionalInputs(index,answerindex,m)" autocomplete="off"  @keydown.enter.prevent @keydown.tab.prevent>
                        </div>
                    </div>
                </span>
            </div>
