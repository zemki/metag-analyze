...
class PasswordSetupTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->martProject = Project::factory()->create([
            'name' => 'Test MART Project for Password Setup',
    ...
    public function it_sends_password_setup_email_for_new_user()
    {
        ...
        $email = 'newuser@example.com';
        $this->postJson('/api/check-email', ['email' => $email, 'project_id' => $this->martProject->id]);
    ...
    public function it_sends_password_setup_email_for_existing_unverified_user()
    {
        ...
        $this->postJson('/api/check-email', ['email' => $user->email, 'project_id' => $this->martProject->id]);
    ...
    public function it_expires_email_check_after_5_minutes()
    {
        ...
        $cacheKey = 'email_check:' . md5(strtolower($email) . $this->app['request']->ip() . $this->martProject->id);
    ...
    public function it_assigns_user_role_to_new_user()
    {
        ...
        // Send password setup
        $this->postJson('/api/send-password-setup', [
            'email' => $email,
            'project_id' => $this->martProject->id,
        ]);
    ...
    public function password_setup_flow_integration_test()
    {
        ...
        $checkResponse = $this->postJson('/api/check-email', ['email' => $email, 'project_id' => $this->martProject->id]);
        $checkResponse->assertStatus(200)
            ->assertJson(['exists' => false]);

        // Step 2: Request password setup email
        $setupResponse = $this->postJson('/api/send-password-setup', [
            'email' => $email,
            'project_id' => $this->martProject->id,
        ]);
